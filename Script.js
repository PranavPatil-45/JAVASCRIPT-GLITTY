...
let container = document.getElementById('WoodCases');
...
function CardList(data){
    ...
    let CardData=data.map((el)=>Card(el.images,el.title)).join("");
    container.innerHTML += CardData;
    console.log(data);

}

function Card(images,title){

    let WDcase =`
     <div class="col-6 col-md-3">
    
      <a href="" style="text-decoration:none;color:#243045"><img src="${images}" class="img-fluid rounded shadow-sm" alt="Walnut">
...
let LeatherContainer = document.getElementById('LeatherCases')
...
function LeatherCardList(data){
     ...
     let LeatherCardData= data.map((e)=>LeatherCard(e.images,e.title)).join("");
    LeatherContainer.innerHTML = LeatherCardData;
    console.log(data);
}

function LeatherCard(images,title){
...
let MacBookCaseContainer = document.getElementById('caseContainer');
...
function MacBookCaseCardList(data){
    ...
    let MacBookCaseData = data.map((el)=>MacBookCaseCard(el.images, el.title, el.ratings, el.reviews, el.price)).join("");
    MacBookCaseContainer.innerHTML=MacBookCaseData;
    console.log(MacBookCaseData)
}
function MacBookCaseCard(images,title,ratings,price,reviews){
    ...
    <div class="col-sm-6 col-md-4 mb-4">
        <div class="border p-3 h-100 position-relative">
          ...
          <div class="text-warning mb-1">
            <p>⭐⭐⭐⭐⭐</p> <small class="text-muted">(${price})</small>
          </div>
...
let MacBookSkinContainer = document.getElementById('skinContainer')
...
function MAcBookSkinCardList(data){
    ...
    let MacBookSkinData= data.map((el)=>MacBookCaseCard(el.images, el.title, el.ratings, el.reviews, el.price)).join("");

    MacBookSkinContainer.innerHTML= MacBookSkinData

}

function MacBookSkinCard(images,title,ratings,reviews,price){
...
let MacAccessoriesContainer = document.getElementById('AccessoriesContainer')
...
function MacAccessoriesCardList(data){
    ...
    let MacAccessoriesCardData = data.map((el)=>
        MacAccessoriesCard(el.images, el.title, el. ratings, el.reviews, el.price)).join("");
    MacAccessoriesContainer.innerHTML=MacAccessoriesCardData;

}

function MacAccessoriesCard(images,title,ratings,reviews,price){

    let MacAccessoriesHTML=`
    <div class="col-6 col-md-3">
      <div class="border">
        <a href="" style="text-decoration:none;color:black;"><img src="${images}" alt="${title}" class="img-fluid">
        <div class="p-2">
...
let IphoneCaseContainer = document.getElementById('IphoneCaseContainer');
...
function IphoneCaseCardList(data){
    ...
    let IPhoneCaseData= data.map((el)=>IphoneCaseCard(el.images, el.title, el.ratings, el.reviews, el.price)).join("");

    IphoneCaseContainer.innerHTML=IPhoneCaseData;
}

function IphoneCaseCard(images,title,ratings,reviews,price)
{
    let IphoneCaseHTML=`
     <div class="col-6 col-md-3">
      <div class="border">
...
let IpadSkinContainer = document.getElementById('IpadSkinContainer');
...
function IpadSkinCardList(data){
    ...
    let IpadSkinData = data.map((el)=>IpadSkinCard(el.images, el.title, el.ratings, el.reviews, el.price)).join("")
    IpadSkinContainer.innerHTML=IpadSkinData;

}


function IpadSkinCard(images,title,ratings,reviews,price){
